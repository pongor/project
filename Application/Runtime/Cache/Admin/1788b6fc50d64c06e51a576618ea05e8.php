				...
				<table class="mod_table">
					...
					<tbody class="mod_tbody targettbody">
						...
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
							<td><input class="cell_inputLang" type="text" placeholder="请输入广告名" value="<?php echo ($data["title"]); ?>" onchange="update(this.value,<?php echo ($data['id'] ? $data['id'] : 0); ?>,'title')"></td>
							<td><!--上传的图片--><img class="banner_advertising" id="img_<?php echo ($data["id"]); ?>" src="<?php echo ($data["file"]); ?>" /></td>
