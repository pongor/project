...
//				alert( (e + P > a(document).height() ? e + P : a(document).height())   + ' and ' +( document.body.clientHeight - a(".dw-persp .dw").height()));
		...
		n.show = function(j) {
			...
			var b = 0,
				e, f = "";
			T && !j && (f = "dw-" + T + " dw-in");
			for (var d = '<div class="dw-trans ' + p.theme + " dw-" + p.display + '">' + ("inline" == p.display ? '<div class="dw dwbg dwi"><div class="dwwr">' : '<div class="dw-persp"><div class="dwo"></div><div class="dw dwbg ' + f + '"><div class="dw-arrw"><div class="dw-arrw-i"><div class="dw-arr"></div></div></div><div class="dwwr">' + (p.headerText ? '' : "")), j = 0; j < p.wheels.length; j++) {
				...
				for (i = 0; 3 > i; i++)
					if (i == q.y) {
						...
						var J = H.getFullYear(),
							Z = L.getFullYear();
						for (h = J; h <= Z; h++){
							g[f.yearText][h] = B.match(/yy/i) ? h : (h + "").substr(2, 2);
							g[f.yearText][h] += '年';
						} 
					} else if (i == q.m) {
						s++;
						g[f.monthText] = {};
						for (h = 0; 12 > h; h++) {
							J = B.replace(/[dy]/gi, "").replace(/mm/, 9 > h ? "0" + (h + 1) : h + 1).replace(/m/, h + 1), g[f.monthText][h] = J.match(/MM/) ? J.replace(/MM/, '<span class="dw-mon">' + f.monthNames[h] + "</span>") : J.replace(/M/, '<span class="dw-mon">' + f.monthNamesShort[h] + "</span>");
							g[f.monthText][h] += '月';
						}
					} else if (i == q.d) {
						s++;
						g[f.dayText] = {};
						for (h = 1; 32 > h; h++){
							g[f.dayText][h] = B.match(/dd/i) && 10 > h ? "0" + h : h; 
							g[f.dayText][h] += '日';
						}
					}
				C.push(g)
